      ...
      */}}
      ...
      initContainers:
        ...
        - name: image-pull-metadata-block
          image: {{ .Values.global.registry }}{{ .Values.singleuser.networkTools.image.name }}:{{ .Values.singleuser.networkTools.image.tag }}
        ...
        - name: image-pull-singleuser
          image: {{ .Values.global.registry }}{{ .Values.singleuser.image.name }}:{{ .Values.singleuser.image.tag }}
          command:
            - /bin/sh
            - -c
            - echo "Pulling complete"
        ...
        - name: image-pull-singleuser-init-and-extra-containers-{{ $k }}
          image: {{ .Values.global.registry }}{{ $container.image }}
          command:
            - /bin/sh
            - -c
            - echo "Pulling complete"
        ...
        - name: image-pull-singleuser-profilelist-{{ $k }}
          image: {{ .Values.global.registry }}{{ $container.kubespawner_override.image }}
          command:
            - /bin/sh
            - -c
            - echo "Pulling complete"
        ...
        - name: image-pull-{{ $k }}
          image: {{ .Values.global.registry }}{{ $v.name }}:{{ $v.tag }}
          command:
            - /bin/sh
            - -c
            - echo "Pulling complete"
      ...
      containers:
        - name: pause
          image: {{ .Values.global.registry }}{{ .Values.prePuller.pause.image.name }}:{{ .Values.prePuller.pause.image.tag }}
          {{- with .Values.prePuller.resources }}
          resources:
            {{- . | toYaml | nindent 12 }}
          {{- end }}
